...
| # | 事项 | 状态 | 说明 |
|---|------|------|------|
| B1 | `automation-runner.ts` typecheck 错误收尾 | ✅ 已解 | 靠放宽 server tsconfig(`noImplicitAny`/`strictNullChecks` off),非 `@ts-nocheck`;typecheck 全绿 |
| B2 | `automation-runner.ts` 领域拆分 | ⏸ 部分完成后止步 | 已抽 6 个子模块(6665→6077 行);剩余 ~4600 行是内聚编排核心,逐域硬拆会造成大量回环 import,暂停 |
| B3 | 接入真实大模型替换 mock 内容生成 | ✅ 已解(交互式路径) | 仅 `POST /plan/:productId` 生效;无人值守批量刻意保持规则化,见 [content-generation.md](content-generation.md) |
| B4 | 本地 JSON 存储 → 轻量数据库 | ✅ 已解 | `node:sqlite`(零新依赖),`PLANNER_STATE_PATH` 兼容保留 |
| B5 | 批量发布前检查 / 批量版本恢复 | ⬜ 未做 | 首次是小批量(limit=3→几十),不挡 |

> 冲刺主线(A 段)的最新卡点已不是本表,而是真实写链路的「阻塞墙」:墙 4 主题颜色/变种行(待写适配)、墙 3 图片恢复(已写待真跑验证)、墙 2 image-editor 第二层(临时绕过中)。见 [blocking-walls-diagnosis.md](blocking-walls-diagnosis.md)。
