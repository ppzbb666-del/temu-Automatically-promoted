...
AI 编排目前仍为 mock（[`packages/shared/src/mock.ts`](packages/shared/src/mock.ts)），真正接大模型是"下一阶段"。
...
- 仓库已初始化（`master` 分支），首提交 `4de04c5 chore: import existing monorepo as baseline`（97 files / 42732 insertions）。
- 已配置 remote：`origin` → `https://github.com/ppzbb666-del/Auto_goods.git`。
