...
assert.match(blockedWithoutProfile.lastError ?? "", /未配置浏览器配置路径/, "missing browser profile startup block should explain the profile requirement")
...
assert.match(blockedWithoutInitializedProfile.lastError ?? "", /浏览器配置目录不存在|初始化/, "uninitialized browser profile startup block should explain the headed profile setup requirement")
