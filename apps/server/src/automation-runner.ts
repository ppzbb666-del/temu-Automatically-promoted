...
const getManualBudgetTrialHistoryPath = () => process.env.MANUAL_BUDGET_TRIAL_HISTORY_PATH ?? path.join(getRepoRoot(), ".runtime/data/manual-budget-trials.json");
...
const RECOVERY_FAILURE_ALERT_THRESHOLD = 2;
const RECOVERY_RELEASED_RETRY_BATCH_LIMIT = 1;
const DEFAULT_PROFILE_LOCK_STALE_MS = 12 * 60 * 60 * 1000;
const DEFAULT_REAL_CALIBRATION_STALE_MS = 24 * 60 * 60 * 1000;
const allowDianxiaomiSmokeCalibration = () => process.env.ALLOW_DIANXIAOMI_SMOKE_URLS === "true";
...
const getProfileLockStaleMs = () => clampInteger(Number.parseInt(process.env.PROFILE_LOCK_STALE_MINUTES ?? "", 10), DEFAULT_PROFILE_LOCK_STALE_MS / 60_000, 5, 7 * 24 * 60) * 60_000;
const getRealCalibrationStaleMs = () => clampInteger(Number.parseInt(process.env.REAL_DIANXIAOMI_CALIBRATION_STALE_MINUTES ?? "", 10), DEFAULT_REAL_CALIBRATION_STALE_MS / 60_000, 30, 7 * 24 * 60) * 60_000;
const formatDurationCompact = (durationMs) => {
...
const averagePerProduct = (total, productCount) => productCount > 0 ? total / productCount : 0;
...
const manualBudgetProofSources = new Set([
    "publish-outcome",
    "repair-plan",
    "failure-diagnosis"
]);
const normalizeManualBudgetProofText = (value) => typeof value === "string" ? value.trim() : "";
const normalizeManualBudgetProofNumber = (value, fallback = 0) => typeof value === "number" && Number.isFinite(value) ? value : fallback;
...
const profileLockAuditId = (action, profilePath, fileName, mtime) => `profile-lock-${createHash("sha1")
    .update([action, profilePath, fileName, mtime ?? "unknown"].join("\0"))
    .digest("hex")
    .slice(0, 12)}`;
const normalizeProfileLockAuditEntry = (entry) => {
...
const loadProfileLockAuditLedger = () => {
    ...
    try {
        ...
        profileLockAuditLedger = entries
        ...
        profileLockAuditLedger = [];
    }
};
const profileLockAuditSummary = (profilePath) => {
    const entries = profilePath
        ? profileLockAuditLedger.filter((entry) => entry.profilePath === profilePath)
        : [];
    return {
...
const cloneManualBudgetProofAutomationMeasurement = (measurement) => ({
    ...measurement,
    reportIds: [...measurement.reportIds],
    reportPaths: [...measurement.reportPaths]
});
const cloneManualBudgetProofRecord = (record) => ({
    ...record,
    baseline: {
        ...record.baseline
    },
...
const normalizeManualBudgetProofAutomationMeasurement = (measurement) => {
...
const measureAutomationReports = (reportPaths) => {
...
const loadManualBudgetProofLedger = () => {
    const ledgerPath = getManualBudgetProofLedgerPath();
    if (!existsSync(ledgerPath)) {
        return;
    }
...
const dryRunJobs = new Map();
const repairPreviewJobs = new Map();
const repairApplyJobs = new Map();
const fillDraftJobs = new Map();
const saveDraftJobs = new Map();
const submitListingJobs = new Map();
const fullFlowJobs = new Map();
const recoveryRuns = new Map();
let recoveryReleases = [];
let manualBudgetProofLedger = [];
...
const readAutomationExecutionReport = (filePath) => {
...
const resolveQueueDaemonProfilePath = () => resolveProfilePath(queueDaemonState.input.profile);
...
const recordProfileLockAuditEntries = (records) => {
    ...
    for (const record of records) {
        if (profileLockAuditLedger.some((entry) => entry.id === record.id)) {
            continue;
        }
        profileLockAuditLedger.unshift(record);
        changed = true;
    }
    if (!changed) {
        return;
    }
    profileLockAuditLedger = profileLockAuditLedger
        .sort((left, right) => right.recordedAt.localeCompare(left.recordedAt))
        .slice(0, PROFILE_LOCK_AUDIT_LIMIT);
...
const collectQueueDaemonAuditEntries = (limit = 10) => queueDaemonState.ticks
    .slice(0, limit)
...
export const getDianxiaomiQueueDaemonHealth = (input: AutomationDryRunStartInput = {}) => {
    ...
    if (queueDaemonState.consecutiveFailures > 0) {
        issues.push({
            id: "consecutive-failures",
            level: queueDaemonState.consecutiveFailures >= queueDaemonState.maxConsecutiveFailures ? "block" : "warning",
            message: `consecutive failures ${queueDaemonState.consecutiveFailures}/${queueDaemonState.maxConsecutiveFailures}`
        });
    }
    if (queueDaemonState.lastError) {
        issues.push({
            id: "last-error",
            level: "warning",
            message: queueDaemonState.lastError
        });
    }
    if (!profile.path) {
        issues.push({
    ...
    if (profile.lockFiles.length > 0 && !queueDaemonState.running) {
        issues.push({
    ...
    if (profile.staleLockFiles.length > 0 && !queueDaemonState.running) {
        issues.push({
    ...
    if (workItems.ready === 0 && browserRecoveryCandidates === 0 && releasedBrowserRecoveryCandidates === 0 && queueDaemonState.status === "ACTIVE") {
        issues.push({
    ...
    const alerts = queueDaemonHealthAlerts(issues, {
        daemonStatus: queueDaemonState.status,
    ...
    return {
        ...
        queue: {
            daemonStatus: queueDaemonState.status,
            running: queueDaemonState.running,
            consecutiveFailures: queueDaemonState.consecutiveFailures,
            maxConsecutiveFailures: queueDaemonState.maxConsecutiveFailures,
            nextRunAt: queueDaemonState.nextRunAt,
            lastError: queueDaemonState.lastError,
            lastFailedCategory: lastFailedTick?.category ?? null
        },
        workItems: {
            ...workItems,
...
const unattendedBrowserProfileStartupCheck = (profile) => {
    if (!profile.path) {
        return {
            ...
            label: "browser profile",
            status: "block",
            message: "browser profile path is not configured; configure a persistent Dianxiaomi profile before unattended startup"
        };
    }
    if (!profile.exists) {
        return {
            ...
            label: "browser profile",
            status: "block",
            message: `browser profile directory is missing: ${profile.path}; initialize and log into Dianxiaomi in headed mode before unattended startup`
        };
    }
    if (profile.lockFiles.length > 0) {
        return {
            ...
            label: "browser profile",
            status: "block",
            message: `profile has possible lock file(s): ${profile.lockFiles.join(", ")}`
        };
    }
    return {
        id: "browser-profile",
        label: "browser profile",
        status: "pass",
        message: `profile ready: ${profile.path}`
    };
};
...
export const getDianxiaomiUnattendedStartupCheck = (input: AutomationDryRunStartInput = {}) => {
    ...
    const checks = [
        {
            ...
            message: runnableWorkItems > 0
                ? `${health.workItems.ready} ready item(s), ${health.workItems.autoRetryCandidates} safe retry candidate(s), ${health.workItems.browserRecoveryCandidates} browser recovery candidate(s), ${health.workItems.releasedBrowserRecoveryCandidates} released retry candidate(s)`
                : "no Dianxiaomi work items are ready for automation"
        },
        {
            id: "selector-config",
            label: "selector config",
            status: selectorGate.selectorReady ? "pass" : "block",
            message: selectorGate.selectorReady ? "selector config is valid" : selectorBlockReason(selectorGate.selectorBlockers),
    ...
    return {
        ...
        recommendedAction: status === "blocked"
            ? "resolve blocked startup checks before starting unattended queue"
            : status === "warning"
                ? "queue can start, but review warnings first"
...
const persistQueueDaemonState = () => {
    ...
    const persisted = cloneQueueDaemonState({
        ...queueDaemonState,
        running: false,
        nextRunAt: queueDaemonState.status === "ACTIVE" ? queueDaemonState.nextRunAt : null
    });
...
const loadQueueDaemonState = () => {
    ...
    try {
        queueDaemonState = normalizeQueueDaemonState(JSON.parse(readFileSync(statePath, "utf8")));
    }
    catch {
        queueDaemonState = defaultQueueDaemonState();
    }
};
...
const scheduleQueueDaemonNextRun = () => {
    if (queueDaemonTimer) {
        ...
        queueDaemonTimer = null;
    }
    if (queueDaemonState.status !== "ACTIVE") {
        queueDaemonState = {
            ...queueDaemonState,
            nextRunAt: null
        };
    ...
    const nextRunAt = new Date(Date.now() + queueDaemonState.intervalSeconds * 1000).toISOString();
    queueDaemonState = {
        ...queueDaemonState,
        nextRunAt
    };
    queueDaemonTimer = setTimeout(() => {
        void tickDianxiaomiQueueDaemon();
    }, queueDaemonState.intervalSeconds * 1000);
    persistQueueDaemonState();
};
...
const recoverQueueDaemonFlowOutcomes = () => {
    const alreadyResolved = new Set(queueDaemonState.resolvedFlowJobIds);
    const outcomes = [];
    for (const flowJobId of queueDaemonState.trackedFlowJobIds) {
    ...
    queueDaemonState = {
        ...
        resolvedFlowJobIds: Array.from(new Set([
            ...queueDaemonState.resolvedFlowJobIds,
            ...outcomes.map((outcome) => outcome.flowJobId)
        ])).slice(-500),
        flowOutcomes: [
            ...outcomes,
            ...queueDaemonState.flowOutcomes
        ].slice(0, 100)
    };
    persistQueueDaemonState();
    return outcomes;
...
const queueDaemonStartupBlock = (input: any = {}) => {
    ...
    const selectorGate = getSelectorReadinessGate(queueDaemonState.input);
...
export const tickDianxiaomiQueueDaemon = async () => {
    ...
    try {
        ...
        if (startupBlock) {
            ...
            queueDaemonState = {
                ...queueDaemonState,
                status: shouldPause ? "PAUSED" : queueDaemonState.status,
                running: false,
                consecutiveFailures,
        ...
        const validationRun = startQueueDaemonValidationRerun(queueDaemonState.input);
        if (validationRun) {
        ...
        const recoveryRun = startQueueDaemonRecoveryRun(queueDaemonState.input);
        if (recoveryRun) {
            const finishedAt = new Date().toISOString();
            const releasedReason = autoRetryReleasedIds.length > 0
                ? `released ${autoRetryReleasedIds.length} non-browser auto-recovery item(s)`
        ...
        const queueRun = startDianxiaomiQueueRun(queueDaemonState.input, {
        ...
        const consecutiveFailures = classification.countsAsFailure ? queueDaemonState.consecutiveFailures + 1 : 0;
        const shouldPause = consecutiveFailures >= queueDaemonState.maxConsecutiveFailures;
        const finishedAt = new Date().toISOString();
        const tick = {
            id,
...
export const getDianxiaomiQueueDaemonState = () => cloneQueueDaemonState(queueDaemonState);
