...
- 启用媒体工具：image translation / batch resize / white background / image editor
...
| 媒体处理 | unattended-apply（启用 4 个工具） | plan-only / 单工具 / 反馈逐次验证 |
...
浏览器插件加载 `apps/extension/dist`（先 `npm run build --workspace @temu-ai-ops/extension`）。
...
- [docs/current-status.md](docs/current-status.md) — 最近改了什么（82KB 流水账）
...
- **Git**：已初始化，首提交 `4de04c5`；当前 3 个 commit 落在 `master` 分支
